 ...
 * Load configuration
 *********************************************************/
let Away = global.Away = require('./plugins/utils');
...
try {
	...
	Away.Init.logs();
	if (Config.inOpenshift === true) Away.Init.logs('forOpenshift');
	Away.Init.config();
	if (Config.inOpenshift === true) Away.Init.config('forOpenshift');
	Away.Init.syncConfigFile();
...
if (Config.watchconfig) {
	...
	fs.watchFile(configPath, (curr, prev) => {
		...
		try {
			...
			console.log('Recarga de config/config.js');
		} catch (e) {
			console.log('Error al cargar config/config.js: ' + e.stack);
		}
	});
}
